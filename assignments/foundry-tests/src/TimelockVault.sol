...
contract TimelockVault {
  ...
  function getVault(
    address _user,
    uint _vaultId
  ) external view returns (uint balance, uint unlockTime, bool active, bool isUnlocked) {
  ...
  function getActiveVaults(
    ...
    external
    view
    returns (uint[] memory activeVaults, uint[] memory balances, uint[] memory unlockTimes)
  {
